...
> [!IMPORTANT]
> API keys entered via the initial setup screen are stored in **plain text** within a JSON file in your OS's default configuration directory (e.g. `~/.config/zizou-nodejs/config.json`). They are **not encrypted**.
